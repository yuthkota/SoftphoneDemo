...
function addCallLog(method, targetPhone, status, timestamp, details = "", borrowerName = "") {
  ...
  const statusBadgeClass = status === "initiated" ? "bg-success" : status === "failed" ? "bg-danger" : "bg-secondary"
    ...
    <div class="card-body">
      <div class="d-flex justify-content-between align-items-center">
        <div class="flex-grow-1">
          ...
          ${details ? `<br><small class="text-muted fst-italic">${details}</small>` : ""}
        </div>
...
async function setupDeviceAndMic() {
  try {
    ...
    device.on("ready", () => {
      ...
      document.getElementById("connectionStatus").className = "status-indicator text-success"
    })
    ...
    device.on("error", (err) => {
      ...
      document.getElementById("connectionStatus").className = "status-indicator text-danger"
    })
...
function startCall() {
  ...
  // Find borrower info
  const account = loanAccounts.find((acc) => acc.phoneNumber === targetPhone || acc.alternatePhone === targetPhone)
...
function toggleMute() {
  ...
  muteBtn.innerHTML = isMuted ? '<i class="bi bi-mic me-2"></i>Unmute' : '<i class="bi bi-mic-mute me-2"></i>Mute'
}
            ...
            <div class="col-md-6">
              <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex align-items-center gap-2">
                  ...
                  <button class="btn btn-success btn-sm" onclick="callAccount('${account.phoneNumber}', '${account.borrowerName}')">
                    <i class="bi bi-telephone"></i>
                  </button>
                </div>
              </div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("addAccountForm").addEventListener("submit", (e) => {
    ...
    const newAccount = {
      ...
      loanAmount: Number.parseFloat(document.getElementById("loanAmount").value),
      outstandingBalance: Number.parseFloat(document.getElementById("outstandingBalance").value),
